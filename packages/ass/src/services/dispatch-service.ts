...
const sendDefence = async (agent: IAgent | IGroup, services: IEnvServices) => {
    ...
    const messageRadius = planEffects[agent.reactedTo].messageRadius;
    ...
    const receivers = await redisServices.geoSearch(agent.actual, messageRadius, agent) as Array<any>;
