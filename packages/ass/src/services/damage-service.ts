...
const damageAgent = async (sender: IDefenseAgent, agentAction: string, receivers:IAgent[], _services: IEnvServices) => {
    const equipment = await pickEquipment(sender,agentAction)
...
const pickEquipment = async (agent: IDefenseAgent, agentAction: string) => {
